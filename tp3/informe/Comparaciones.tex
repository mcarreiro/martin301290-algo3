...
Como se puede observar en los gráficos, el GRASP se mantiene dominante (dentro de los algoritmos eurísticos) con la menor cantidad de nodos en su conjunto resultado.
...
eso es bastante claro ya que habrá más nodos cerca del grado = n-1, por lo que con pocos nodos se encuentra el conjunto dominante. Además una vez 
que la probabilidad 
se vuelve muy alta
muchos de los nodos del grafo llegan a tener nivel n-1 y para esos grafos todos los algoritmos devuelven el mismo resultado que además es óptimo.\\
Otro aspecto importante en cuanto al valor de la probabilidad es que cuanto menor sea esta más nodos contiene el conjunto dominante en todos los resultados. Esto es 
debido a que al haber manos aristas es lógico que se necesiten mas nodos dominantes para cubrir todo el grafo (ejemplo claro es el caso del grafo completamente disconexo
, osea sin arsitas, que requiere que todos los nodos del grafo esten en el CD.\\
...
Aquí podemos observar la abismal diferencia entre el algoritmo exacto y todas las eurísticas. El exacto desaparece del gráfico para grafos de menos de 25 nodos
lo que evidencia el porque de la imposibilidad de correr dicho algoritmo para 30 o mas nodos. Algo curioso es la bajada que hace este algoritmo antes de desaparecer,
eso probablemente sea porque en el grafo recibido existía un nodo con grado n -1 y por esto al encontrarlo lo devolvio y corto el procedimiento (como ya explicamos al principio del informe).
